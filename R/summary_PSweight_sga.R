...
summary.PSweight_sga<-function(object,contrast=NULL,type='DIF',het=FALSE,...){
  ...
  if(het){
    ...
    het_evalout <- do.call(rbind, het_eval)
  }

  estimate <-do.call(rbind, estimates)
  rownames(estimate) <- rownames(muhat)
  out<-list(estimates=estimate, contrast=contrast,group=group,trtgrp=trtgrp,het_eval=het_evalout,method=method)
